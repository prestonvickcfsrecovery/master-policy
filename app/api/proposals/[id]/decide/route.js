...
export async function POST(req, { params }) {
  ...
  try {
    ...
    // A "new policy" proposal creates its own entry, so approved policies land in
    // the Policy tab as searchable entries rather than being buried in another one.
    if (approve && p.kind === "new" && p.proposed_text && p.proposed_text.trim()) {
      const code = (p.new_code || "").trim().toUpperCase();
      const section = (p.new_section || "universal").trim();
      let id = code.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || `policy-${p.id}`;

      const { rows: taken } = await client.query(`SELECT id FROM entries WHERE id = $1`, [id]);
      if (taken.length) id = `${id}-${p.id}`;

      // place it at the end of its section
      ...
      await client.query(
        `INSERT INTO entries (id, code, title, summary, body, section, ord, zone, status,
         ...
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,'approved','[]'::jsonb,$9,1,$10)`,
        [id, code, (p.new_title || p.title).slice(0, 200), (p.new_summary || "").slice(0, 400),
         p.proposed_text, section, ord, section === "front" ? null : section,
         `Added by proposal #${p.id}`, user.email]
      );
      await client.query(
        ...
        [id, p.proposed_text, user.email, p.id, note || ("New policy: " + (p.new_title || p.title))]
      );
      createdId = id;
      applied = 1;
